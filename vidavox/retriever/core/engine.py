    ...
    def __init__(
        ...
        *,
        ...
        self.retriever = RetrieverFactory.create(
            kind=kind, embedding_model=self.embedding_model)

        self.bm25_wrapper = getattr(self.retriever.model, "bm25_search", None)
        self.faiss_wrapper = getattr(
            self.retriever.model, "faiss_search", None)
        self.hybrid_search = getattr(self.retriever, "model", None)
