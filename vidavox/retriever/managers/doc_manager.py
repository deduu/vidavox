...
class DocumentManager:
    ...
    def get_documents(
        ...
        with self.lock:
            base = set(self.user_to_doc_ids.get(
                user_id, set())) | self.shared_doc_ids
            if folder_id:
    ...
    def get_all_just_added_documents(
        ...
        with self.lock:
            ...
            result = {
                doc_id: self.documents[doc_id].text for doc_id in just_ids}
            if clear_after:
                self._just_added[user_id].clear()
            return result
    ...
    def get_doc_ids_by_prefix(
        ...
        """
        with self.lock:
            all_ids = self.user_to_doc_ids.get(
                user_id, set()) if user_id else self.shared_doc_ids
            filtered_ids = [
    ...
    def clear_user(self, user_id: str) -> None:
        with self.lock:
            ...
            for doc_id in doc_ids:
                used_elsewhere = any(
                    doc_id in docs for docs in self.user_to_doc_ids.values())
