...
class Hybrid_search:
    def __init__(
        ...
        search_mode: SearchMode = SearchMode.BM25
    ):
    ...
    def advanced_search(
        ...
        bm25_scores, bm25_doc_ids = self._get_bm25_results(
            keywords_str, top_n, prefixes, include_doc_ids, exclude_doc_ids)
        faiss_distances, faiss_doc_ids = self._get_faiss_results(
            query, top_n, prefixes, include_doc_ids, exclude_doc_ids)

        bm25_scores_dict, faiss_scores_dict = self._map_scores_to_doc_ids(
            bm25_doc_ids, bm25_scores, faiss_doc_ids, faiss_distances
        )
        all_doc_ids = sorted(set(bm25_doc_ids).union(faiss_doc_ids))
        filtered_doc_ids = self._filter_doc_ids_by_prefixes(
            all_doc_ids, prefixes)
        if not filtered_doc_ids:
            self.logger.info("No documents match the prefixes.")
            return []
        ...
        hybrid_scores = self._calculate_hybrid_scores(
            bm25_scores_normalized, faiss_scores_normalized)
        results = self._get_top_n_results(
            filtered_doc_ids, hybrid_scores, top_n, threshold)
        return results

    def advanced_search_batch(
        self,
        queries: List[str],
        # optional per-query keywords
        ...
        bm25_batch = self._get_bm25_results_batch(
            [" ".join(kws) if kws else "" for kws in (
                keywords_list or [[]]*len(queries))],
    ...
    def _get_faiss_results_batch(
        ...
        id_filter = self._build_faiss_filter(
            prefixes, include_ids, exclude_ids)
    ...
    def _build_faiss_filter(self, prefixes, include_ids, exclude_ids):
        ...
        if prefixes:
            allowed &= {d for d in allowed if any(
                d.startswith(p) for p in prefixes)}
        if include_ids:
    ...
    def _get_faiss_results(self,
        ...
        dists, ids = self.faiss_search.search(
            query, k=top_k, id_filter=id_filter)
    ...
    def _get_filtered_scores(
        ...
        for doc_id in filtered_doc_ids:
            ...
            default_faiss = max(faiss_scores_dict.values()) + \
                1 if faiss_scores_dict else 1
            faiss_aligned_scores.append(
                faiss_scores_dict.get(doc_id, default_faiss))
        # Invert FAISS distances so that higher values mean higher similarity.
        faiss_aligned_scores = [1 / score if score !=
                                0 else 0 for score in faiss_aligned_scores]
    ...
    def _normalize_scores(
        ...
        bm25_scores_normalized = self._normalize_array(
            filtered_bm25_scores, scaler_bm25)
        scaler_faiss = MinMaxScaler()
        faiss_scores_normalized = self._normalize_array(
            filtered_faiss_scores, scaler_faiss)
    ...
    def _normalize_array(self, scores: List[float], scaler: MinMaxScaler) -> np.ndarray:
        ...
        if np.ptp(scores_array) > 0:
            normalized_scores = scaler.fit_transform(
                scores_array.reshape(-1, 1)).flatten()
        else:
    ...
    def _get_top_n_results(
        ...
        if len(threshold_indices) == 0:
            ...
            return []
        sorted_indices = threshold_indices[np.argsort(
            hybrid_scores[threshold_indices])[::-1]]
        top_indices = sorted_indices[:top_n]
        results = [(filtered_doc_ids[idx], hybrid_scores[idx])
    ...
    async def _get_top_n_results_async(
        ...
        loop = asyncio.get_event_loop()
        def sorted_indices_calculation(): return threshold_indices[np.argsort(
            hybrid_scores[threshold_indices])[::-1]]
    ...
    def _rerank_results(self, query: str, results: List[Tuple[str, float]]) -> List[Tuple[str, float]]:
        ...
        document_texts = [self.bm25_search.get_document(
        ...
        with torch.no_grad():
            rerank_scores = self.reranker.compute_score(
                rerank_inputs, normalize=True)
        reranked_results = sorted(
            zip(doc_ids, rerank_scores), key=lambda x: x[1], reverse=True)
    ...
    async def async_remove_document(self, doc_id: str) -> Dict[str, bool]:
        ...
        try:
            ...
            self.logger.warning(
                f"FAISS async failed to remove '{doc_id}': {e}")
